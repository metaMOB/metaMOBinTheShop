...
public class TouchPointPanel extends Panel {
	...
	private String htmlPraefix = "<span class=\"warning\">!</span>";
	...
	public TouchPointPanel(String id, final UserShoppingCart model, final AbstractTouchpoint tp, final IMainPageItemCallback itemPanelCallback) {
		...
		if (SessionUtil.isLoggedIn()){
			...
			messageModel.setObject(htmlPraefix+"Bestellung erst nach Anmeldung möglich!");
		}
    			...
    			if(SessionUtil.isLoggedIn()){
    				...
    				try {
						...
						if (lst.size() < 1){
							...
							messageModel.setObject(htmlPraefix+"Der Artikel "+product.getName()+" ist leider nicht verfügbar!");
